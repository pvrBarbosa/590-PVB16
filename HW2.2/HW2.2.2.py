...
if (INPUT_FILE == "MPG"):
    ...
    #EXPLORE THE DATA
    #IMPORT PROF SEABORN_VISUALIZER SCRIPT
...
#------------------------
def model(x,p):
    #if(model_type=="linear"):   return  p[:-1]*x+p[-1]  
...
def minimizer(f,xi, algo='GD', LR=0.01):
    ...
    max_iter=10000        #MAX NUMBER OF ITERATION
...
NFIT = len(X[0])+1
if model_type == "logistic": LR =5
else: LR = 0.01 

po=np.random.uniform(2,1.,size=NFIT)

#TRAIN MODEL USING SCIPY MINIMIZ 
p_final=minimizer(loss,po, LR = LR)        
