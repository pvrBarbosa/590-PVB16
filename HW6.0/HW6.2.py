...
#-----------------------------------------------------------------------------
EPOCHS          =   35
NKEEP           =   60000        #MNIST total = 60000 / CIFAR10 total = 50000
BATCH_SIZE      =   128
DATA            =   "MNIST"
...
plot_orig_rec(model, x_train, "MNIST Reconstruction")
...
def detect_anomaly(model, test_data, trained_data):
    ...
    train_errors = np.mean(msle(train_pred.reshape(trained_data.shape), trained_data).numpy(),axis =1)
    # Calculates the threshold as mean + 4stddev
    threshold = np.mean(train_errors + 4*np.std(train_errors))
        
    test_pred = model.predict(test_data)
    # get the error for each instance
    test_errors = np.mean(msle(test_pred.reshape(test_data.shape), test_data).numpy(),axis =1)
    # 1 = anomaly, 0 = normal
...
predictions = detect_anomaly(model, check_images, x_train)

train_detect = detect_anomaly(model, x_train, x_train)
